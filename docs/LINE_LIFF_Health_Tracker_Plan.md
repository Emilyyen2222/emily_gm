...
### 資料流概觀
```
[Vercel Cron 03:00 UTC = 11:00 台北] --> [/api/cron/daily-reminder]
                                              |
...
*   Cron 設定為 `0 3 * * *`（03:00 UTC）以對應台北時間 11:00。

### 3. 觸發機制 (Trigger)
*   **定時提醒（主要路徑）：** Vercel Cron 每天 11:00（台北）觸發 `/api/cron/daily-reminder`，呼叫 Messaging API push message，將含 LIFF 連結的提醒推送至已登記群組。
...
4.  `vercel.json` 設定：
    ```json
    { "crons": [{ "path": "/api/cron/daily-reminder", "schedule": "0 3 * * *" }] }
    ```
