...
### 資料流概觀
```
[Vercel Cron 01:30 UTC = 09:30 台北] --> [/api/cron/daily-reminder]
                                              |
...
*   Cron 設定為 `30 1 * * *`（01:30 UTC）以對應台北時間 09:30。

### 3. 觸發機制 (Trigger)
*   **定時提醒（主要路徑）：** Vercel Cron 每天 09:30（台北）觸發 `/api/cron/daily-reminder`，呼叫 Messaging API push message，將含 LIFF 連結的提醒推送至已登記群組。
...
4.  `vercel.json` 設定：
    ```json
    { "crons": [{ "path": "/api/cron/daily-reminder", "schedule": "30 1 * * *" }] }
    ```
